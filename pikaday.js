/*!
 * pikaday-time - Pikaday with time support as an ES6 module
 *
 * https://github.com/paulscherrerinstitute/pikaday-time
 *
 * Based on pikaday-time by Owen Mead-Robins
 * https://github.com/owenmead/Pikaday
 *
 * Based on Pikaday by David Bushell
...
renderTimePicker = function(num_options, selected_val, select_class, display_func, increment_by) {
    ...
    var to_return = '<td><div class="styled-select slate"><select class="pika-select '+select_class+'">';
    for (var i = 0; i < num_options; i += increment_by) {
        to_return += '<option value="'+i+'" '+(i==selected_val ? 'selected' : '')+'>'+display_func(i)+'</option>'
    }
    to_return += '</select></div></td>';
    return to_return;
},
